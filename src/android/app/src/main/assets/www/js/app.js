...
function displayOrderDetails(data) {
    if (data.client_info.printer == "1") {
        $("#BtnPrint").css("display","block");
    }

    MerchantTxt = "";
    MerchantTxt += data.merchant_info.merchant_name + "^";
    MerchantTxt += data.merchant_info.merchant_address + "^";
    MerchantTxt += data.merchant_info.merchant_contact_phone + "^";
    ...
    html += '</ons-list-item>';
    CustomerTxt = "";
    CustomerTxt += "Name: " + data.client_info.full_name + "\n";
    if (!empty(data.client_info.contact_phone)) {
        ...
        html += '</ons-col>';
        CustomerTxt += "Phone: " + data.client_info.contact_phone + "\n";
    ...
    if (!empty(data.client_info.car_plate_number)) {
        CustomerTxt += "Car Details" + "\n";
        CustomerTxt += "Car No. " + data.client_info.car_plate_number + "\n";
        CustomerTxt += "Model. " + data.client_info.car_model + "\n";
        CustomerTxt += "Color. " + data.client_info.car_color + "\n";
    }
    ...
    if (data.trans_type_raw == "delivery") {
        ...
        html += '</ons-row>';
        html += '</ons-list-item>';
        CustomerTxt += "Address: " + data.client_info.address + "\n";
    }
    ...
    PaymentTxt = "";
    PaymentTxt += getTrans("Payment Type", 'payment_type') + ": " + data.payment_type + "\n";
    ...
    if (data.payment_type_raw == "PYR" || data.payment_type_raw == "pyr") {
        ...
        PaymentTxt += getTrans("Card#", 'card_number') + ": " + data.payment_provider_name + "\n";
    }
    ...
    if (data.payment_type_raw == "OCR" || data.payment_type_raw == "ocr") {
        ...
        PaymentTxt += getTrans("Card#", 'card_number') + ": " + data.credit_card_number + "\n";
    }

    if (data.trans_type_raw == "delivery") {
        html += TPLorderRow(getTrans("Delivery Date", 'delivery_date'), data.delivery_date);
        PaymentTxt += getTrans("Delivery Date", 'delivery_date') + ": " + data.delivery_date + "\n";
    } else {
        html += TPLorderRow(getTrans("Pickup Date", 'pickup_date'), data.delivery_date);
        PaymentTxt += getTrans("Pickup Date", 'pickup_date') + ": " + data.delivery_date + "\n";
    }

    if (!empty(data.delivery_time)) {
        if (data.trans_type_raw == "delivery") {
            html += TPLorderRow(getTrans("Delivery Time", 'delivery_time'), data.delivery_time);
            PaymentTxt += getTrans("Delivery Time", 'delivery_time') + ": " + data.delivery_time + "\n";
        } else {
    ...
    if (!empty(data.delivery_instruction)) {
        PaymentTxt += getTrans("Delivery Instruction", 'delivery_instructions') + ": " + data.delivery_instruction + "\n";
    }
    if (!empty(data.client_info.location_name)) {
        PaymentTxt += getTrans("Location Name", 'location_name') + ": " + data.client_info.location_name + "\n";
    }
    ...
    if (!empty(data.item)) {
        $.each(data.item, function (key, val) {
            ...
            OrderTxt = ""
            OrderTxt += val.qty + "x " + price + " " + val.item_name;
                ...
                $.each(val.sub_item_new, function (key2, sub_item) {
                    ...
                    if (sub_item.length > 0) {
                        $.each(sub_item, function (key3, sub_items) {
                            ...
                            OrderTxt += t_desc + "      " + sub_items.total + "\n";
                        });
                    }
                });
            }
    ...
    if (!empty(data.total)) {
        ...
        if (!empty(data.total.discounted_amount)) {
            ...
            OrderTxt += getTrans("Discount", 'discount') + " " + data.total.discount_percentage + "          " + data.total.discounted_amount1 + "\n";
            if (!empty(data.total.points_discount)) {
                if (data.total.points_discount > 0) {
                    ...
                    OrderTxt += getTrans("Points Discount", 'point_discount') + "          " + data.total.points_discount1 + "\n";
                }
            }
            ...
            html += TPLorderRow(getTrans("Sub Total", 'sub_total'), data.total.subtotal);
            OrderTxt += getTrans("Sub Total", 'sub_total') + "       " + data.total.subtotal + "\n";
        }
        ...
        if (!empty(data.total.voucher_amount)) {
            if (data.total.voucher_amount > 0) {
                ...
                if (data.total.voucher_type == "percentage") {
                    ...
                    OrderTxt += getTrans("Less Voucher", 'less_voucher') + "    " + data.total.voucher_percentage, data.total.voucher_amount1 + "\n";
                } else {
                    html += TPLorderRow(getTrans("Less Voucher", 'less_voucher'), "(" + data.total.voucher_amount1 + ")");
                    OrderTxt += getTrans("Less Voucher", 'less_voucher') + "    " + data.total.voucher_amount1 + "\n";
                }

                html += TPLorderRow(getTrans("Sub Total (after less voucher)", 'sub_total_after_voucher'), data.total.subtotal2, 'fixed-col');
                OrderTxt += getTrans("Sub Total (after less voucher)", 'sub_total_after_voucher') + "    " + data.total.subtotal2 + "\n";
            }
        }

        if (!empty(data.total.delivery_charges)) {
            html += TPLorderRow(getTrans("Delivery Fee", 'delivery_fee'), data.total.delivery_charges);
            OrderTxt += getTrans("Delivery Fee", 'delivery_fee') + "        " + data.total.delivery_charges + "\n";
            //OrderTxt += getTrans("Delivery Fee", 'delivery_fee') + "        " + (data.total.delivery_charges) + "\n";
        }
        ...
        if (!empty(data.total.merchant_packaging_charge)) {
            ...
            OrderTxt += getTrans("Packaging", 'packaging') + "        " + data.total.merchant_packaging_charge + "\n";
        }

        if (!empty(data.total.taxable_total)) {
            html += TPLorderRow(getTrans("Tax", 'tax') + " " + data.total.tax_amt, data.total.taxable_total);
            OrderTxt += getTrans("Tax", 'tax') + "        " + data.total.tax_amt + "\n";
        }

        if (!empty(data.total.cart_tip_value)) {
            html += TPLorderRow(getTrans("Tips", 'tips'), data.total.cart_tip_value);
            OrderTxt += getTrans("Tips", 'tips') + "        " + data.total.cart_tip_value + "\n";
        }
    }
    OrderTxt += getTrans("Total", 'total') + "        " + data.total.total + "\n";
    html += '<ons-list-header class="header">';
    html += '<ons-row>';
    html += '<ons-col>' + getTrans("Total", 'total') + '</ons-col>';
    html += '<ons-col class="text-right">' + data.total.total + '</ons-col>';
